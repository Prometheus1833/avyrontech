...
type Promotion = {
  ...
  discount_scope: "order" | "annual_subscription";
  active: 0 | 1;
...
const initialForm = {
  ...
  label: "",
  discountPercent: "10",
  discountScope: "order" as "order" | "annual_subscription",
  perUserLimit: "1",
  maxRedemptions: "",
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5 sm:col-span-2"><Label>Domeniu de aplicare</Label><Select value={form.discountScope} onValueChange={(value) => setForm({ ...form, discountScope: value as typeof form.discountScope })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="order">Întreaga valoare eligibilă a comenzii</SelectItem><SelectItem value="annual_subscription">Doar abonamentul selectat pentru 12 luni</SelectItem></SelectContent></Select><p className="text-xs text-muted-foreground">Regula este verificată de Worker la ofertare și din nou la înregistrarea comenzii.</p></div>
          ...
          {items.map((promotion) => (
            <Card key={promotion.id} className={!promotion.active ? "opacity-65" : undefined}>
              ...
              <CardContent className="space-y-3">
                <Badge variant="outline">{promotion.discount_scope === "annual_subscription" ? "Numai abonament anual" : "Comandă eligibilă"}</Badge>
