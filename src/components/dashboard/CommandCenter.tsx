...
export default function CommandCenter({ open, onOpenChange, access, onNavigate }: Props) {
  ...
  const allCommands = [...commands.map(c=>({...c,center:undefined as string|undefined})),...modules.map(m=>({label:m.name,hint:m.detail,section:"os-centers" as SectionId,icon:Command,allowed:true,center:m.id}))];
  const filtered = allCommands.filter((command) => !normalized || normalize(`${command.label} ${command.hint}`).includes(normalized));
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-w-xl overflow-hidden border-white/10 bg-[#0b1020] p-0 text-slate-100 shadow-[0_40px_120px_-30px_rgba(99,102,241,.75)]">
      ...
      <div className="max-h-[55vh] overflow-y-auto p-2">
        ...
        {filtered.map((command) => { const Icon = command.icon; return <button key={command.label} type="button" onClick={() => { if(command.center){onNavigate("os-centers",command.center);onOpenChange(false);} else {navigate(command.section);if(command.label==="Creează lead")setParams(previous=>{const next=new URLSearchParams(previous);next.set("action","new-lead");return next;});} }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition hover:bg-violet-400/[0.08]"><span className="grid size-9 place-items-center rounded-xl bg-violet-400/10 text-violet-300"><Icon className="size-4" /></span><span><span className="block text-sm font-medium text-slate-200">{command.label}</span><span className="block text-xs text-slate-600">{command.hint}</span></span></button>; })}
