...
export default function CurrencySwitch({
  ...
  return (
    <div data-testid="currency-switch" className={`inline-flex flex-col items-center ${className}`}>
      <div className={`inline-flex items-center rounded-full border border-foreground/15 bg-background/75 p-1 shadow-sm backdrop-blur-sm ${compact ? "gap-0" : "gap-0.5"}`}>
        <span aria-hidden className={`grid place-items-center rounded-full text-foreground/55 ${compact ? "size-7" : "size-8"}`}>
          <ArrowRightLeft className={compact ? "size-3" : "size-3.5"} />
        </span>
        {(["EUR", "RON"] as DisplayCurrency[]).map((option) => (
          <button
            ...
            className={`rounded-full font-mono font-bold tracking-[0.14em] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/50 ${
